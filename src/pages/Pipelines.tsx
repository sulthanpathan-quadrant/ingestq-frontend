...
import { cn } from "@/lib/utils";
import type { Job } from "@/components/types/jobs";
import type { Pipeline as FrontendPipeline, Edge } from "@/components/types/pipeline";
import { Node, Position, MarkerType } from "reactflow";
import { createPipeline, runPipeline, editPipeline } from "@/lib/api"; // Adjust the import path as needed

// Define the backend Pipeline interface
...
// Placeholder getAuthToken function
// Replace with your actual authentication logic
const getAuthToken = (): string => {
  const token = localStorage.getItem('auth_token') || 'your-auth-token-here';
  if (!token) {
...
export default function Pipelines() {
  const { toast } = useToast();
  const [jobs, setJobs] = useState<Job[]>([]);
  const [pipelines, setPipelines] = useState<FrontendPipeline[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredPipelines, setFilteredPipelines] = useState<FrontendPipeline[]>([]);
  const [pipelineBuilderOpen, setPipelineBuilderOpen] = useState(false);
  const [editingPipeline, setEditingPipeline] = useState<FrontendPipeline | null>(null);
  const [viewingPipeline, setViewingPipeline] = useState<FrontendPipeline | null>(null);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [pipelineToDelete, setPipelineToDelete] = useState<FrontendPipeline | null>(null);
  ...
  const getPipelineStatus = (pipeline: FrontendPipeline) => {
    const pipelineJobs = jobs.filter(j => pipeline.jobs.includes(j.id));
    const statuses = pipelineJobs.map(j => j.status.toLowerCase());
    if (statuses.includes('failed')) return 'Failed';
    if (statuses.includes('running')) return 'Running';
  ...
  const handleEditPipeline = (pipeline: FrontendPipeline) => {
    setEditingPipeline(pipeline);
    setPipelineBuilderOpen(true);
  };

  const handleSavePipeline = async (pipelineData: { name: string; jobs: string[]; nodes: Node[]; edges: Edge[] }) => {
    try {
      let newPipelines: FrontendPipeline[];
      if (editingPipeline) {
        // Call editPipeline API for existing pipelines
        ...
        if (!response.success) {
          throw new Error(response.message || "Failed to edit pipeline");
        }
        const backendPipeline: BackendPipeline = response.pipeline;
        const updatedPipeline: FrontendPipeline = {
          ...editingPipeline,
          id: backendPipeline.pipelineId,
          name: backendPipeline.pipelineName,
          jobs: backendPipeline.pipelineConfig,
          createdAt: backendPipeline.createdAt,
          nodes: pipelineData.nodes,
          edges: pipelineData.edges,
        };
        newPipelines = pipelines.map(p => p.id === editingPipeline.id ? updatedPipeline : p);
        toast({ title: "Pipeline Updated", description: response.message || "Pipeline has been updated successfully." });
      } else {
        // Call createPipeline API for new pipelines
        ...
        const backendPipeline: BackendPipeline = response.pipeline;
        const newPipeline: FrontendPipeline = {
          id: backendPipeline.pipelineId,
          name: backendPipeline.pipelineName,
          jobs: backendPipeline.pipelineConfig,
          createdAt: backendPipeline.createdAt,
        ...
        newPipelines = [...pipelines, newPipeline];
        toast({ title: "Pipeline Created", description: response.message || "Pipeline has been created." });
      }
  ...
  const handleDeletePipeline = (pipeline: FrontendPipeline) => {
    setPipelineToDelete(pipeline);
    setDeleteConfirmOpen(true);
  };
  ...
  const handleRunPipeline = async (pipeline: FrontendPipeline) => {
    try {
      ...
      if (!response.success) {
        throw new Error(response.message || "Failed to run pipeline");
      }
      toast({ title: "Pipeline Started", description: response.message || `${pipeline.name} is now running.` });
      // Optionally update local state with new status or lastRun
      setPipelines(prevPipelines =>
        prevPipelines.map(p =>
          p.id === pipeline.id
            ? { ...p, status: "Running", lastRun: new Date().toISOString() }
            : p
        )
      );
    } catch (error: any) {
  ...
  const handleViewPipeline = (pipeline: FrontendPipeline) => {
    setViewingPipeline(pipeline);
  };
