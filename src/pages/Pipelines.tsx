...
// import { getAllPipelines, runPipeline, editPipeline, createPipeline } from "@/lib/api";

// // Define TypeScript interfaces for type safety
// interface Job {
//   id: string;
//   name: string;
//   stages?: { id: string; name: string; type: string; status: string }[];
//   status?: string;
// }
...
//   num_jobs: number;
//   jobs: Job[];
//   nodes: any[]; // Use specific React Flow Node type if available
//   edges: any[]; // Use specific React Flow Edge type if available
//   status: string;
// }
...
//                 }))
//               : [],
//             nodes: p.nodes || [], // Fallback to empty array
//             edges: p.edges || [], // Fallback to empty array
...
//     try {
//       await runPipeline({ pipelineId: pipeline.id, token: "your-auth-token" }); // Replace with actual token logic
//       toast({ title: "Pipeline Started", description: `${pipeline.pipelineName} is now running.` });
//       await fetchPipelines();
//     } catch (error) {
//       toast({ title: "Error", description: "Failed to run pipeline." });
//       console.error(error);
//     }
//   };
...
//   const handleRunPipeline = async (pipeline: Pipeline) => {
//     try {
//       await runPipeline({ pipelineId: pipeline.id, token: "your-auth-token" });
//       toast({ title: "Pipeline Started", description: `${pipeline.pipelineName} is now running.` });
//       await fetchPipelines();
//     } catch (error) {
...
//   folderName?: string;
//   pipelineName?: string; // Added to store pipeline name
// }
...
//                     status: job.Status || "Pending",
//                     description: job.description || "",
//                     isConnected: job.datadestination ? true : false, // Connected if datadestination exists
//                     pipelineName: pipelineName, // Store pipeline name in job
...
//                           id: `${job.jobId || `job-${i}`}-${name}`,
//                           name,
//                           type: name, // Use step name as type for consistency with ViewJobDialog01
//                           status: status as string,
//                         }))
//                       : [],
//                   }))
...
//                       onClick={() => handleViewPipeline(pipeline)}
//                     >
//                       {pipeline.jobs.filter((job) => job.isConnected).length} {/* Show only connected jobs count */}
//                     </Badge>
//                   </div>
//                   <div className="text-muted-foreground text-sm">{pipeline.createdAt}</div>
//                   <div>
...
//               </DialogHeader>
//               <div className="space-y-4">
//                 {viewingPipeline.jobs.filter((job) => job.isConnected).length === 0 ? (
//                   <div className="text-center py-6">
//                     <p className="text-muted-foreground">No connected jobs in this pipeline</p>
//                   </div>
//                 ) : (
//                   viewingPipeline.jobs
//                     .filter((job) => job.isConnected)
//                     .map((job, index) => (
//                       <Card key={job.id} className="shadow-sm">
//                         <CardContent className="p-4">
//                           <div className="flex items-center gap-4">
...
//                     status: job.Status || "Pending",
//                     description: job.description || "",
//                     isConnected: true, // All jobs are connected by default since they're in the pipeline
//                     pipelineName: pipelineName,
//                     triggerType: job.triggerType || "Unknown",
//                     email: job.email || "",
//                     glue_job_name: job.glue_job_name || "",
...
//   folderName?: string;
//   pipelineName?: string;
//   pipelineId?: string; // Added to store pipeline ID
// }
...
//                     isConnected: true,
//                     pipelineName: pipelineName,
//                     pipelineId: pipelineId, // Store pipeline ID in each job
//                     triggerType: job.triggerType || "Unknown",
//                     email: job.email || "",
//                     glue_job_name: job.glue_job_name || "",
//                     scheduleDetails: job.scheduleDetails || {},
...
//     try {
//       setViewingPipeline(null); // Close any existing dialog first
...
//           id: job.jobId,
//           name: job.jobName,
//           category: "Unknown", // Not provided in API response
//           lastRun: job.LastRun,
//           status: job.Status,
...
//             : [],
//         }));
//         // Update the pipeline's jobs with fresh data
//         setPipelines((prev) =>
//           prev.map((p) =>
//             p.id === pipeline.id
//               ? { ...p, jobs: mappedJobs }
