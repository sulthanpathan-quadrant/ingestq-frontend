  ...
  useEffect(() => {
    const fetchDQRules = async () => {
      ...
      try {
        ...
        // Get input type from localStorage (set during upload)
...
if (inputType === 'database') {
  // Retrieve database connection details from localStorage
  ...
  console.log('Database request payload:', { ...request, db_password: '***' }); // Hide password in logs
} else if (inputType === 'xlsx') {
  // Get sheet name from localStorage
...
const response: DQRulesGenerationResponse = await runDQRulesGeneration(request);
      ...
      setIsLoadingRules(false); // Stop loading in all cases
    }
    };
...
// Get input type from localStorage (same as used in rules generation)
const inputType = localStorage.getItem('input_type') || 'csv';
...
// Add appropriate parameters based on input type
if (inputType === 'database') {
  request.db_config = {
 ...
 const handleSaveRule = () => {
  ...
  acc[analysis.ruleName] = analysis.solution;  // ✅ Just the solution string
  return acc;
}, {} as any);
// Get input type from localStorage (same as used in validation)
const inputType = localStorage.getItem('input_type') || 'csv';
