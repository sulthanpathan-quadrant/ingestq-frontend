...
export default function BusinessLogic() {
 ...
 const handleRunAllRules = async () => {
  ...
  const inputType = localStorage.getItem("input_type") as "csv" | "xlsx" | "parquet" | "database" || "csv"
  
  console.log("🔍 Input Type:", inputType)

  // Build payload based on input_type
  let requestData: BLValidationRequest

  if (inputType === "database") {
    // Database input type - read credentials from localStorage
    ...
    // CSV or Parquet - standard S3 file input
    const s3Bucket = localStorage.getItem("selectedBucket")
    const s3Key = localStorage.getItem("selectedFile")
...
toast({
  ...
  description: `${bodyData.passed_rules} rules passed, ${bodyData.failed_rules} rules failed`,
})
    } else {
