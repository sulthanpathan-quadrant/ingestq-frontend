...
import { getBuckets, getObjects, getFile, getAzureContainers, getAzureBlobs, getAzureBlobFile, getOneLakeWorkspaces, getOneLakeLakehouses, getOneLakeContents, getOneLakeFile, getDatabaseTables, transferInputToS3, getDeltaTables, exportDeltaToS3, getExcelSheets } from "@/lib/api";
...
export default function Upload() {
  ...
  const handleConnect = async (type: "source" | "destination") => {
    if (type === "source") {
      ...
      if (selectedDestination === "s3" || selectedDestination === "azure-blob" || selectedDestination === "onelake" || selectedDestination === "delta-tables") {
        ...
        try {
          if (selectedSource === "delta-tables") {
            ...
            if (exportResponse.success && exportResponse.s3_path) {
              ...
              localStorage.setItem("input_type", "parquet"); // Delta tables export as parquet
            ...
            // Existing transfer logic for other sources (local, azure-blob, onelake)
            ...
            const storageType: "blob" | "onelake" | "local" =
              selectedSource === "azure-blob" ? "blob" :
                selectedSource === "onelake" ? "onelake" :
                  "local";
            ...
            if (transferResponse.status === 'success') {
              ...
              localStorage.setItem("org_data_source", selectedSourcePath); // ← ADD THIS LINE
              ...
              //   description: `File staged to S3 successfully${sheetName ? ` (Sheet: ${sheetName})` : ''}`
              // });
            }
          }
        } catch (error) {
