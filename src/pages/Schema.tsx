...
import { Database, ArrowRight, Plus, Minus, AlertTriangle, CheckCircle, Eye, Network, ArrowLeft, RefreshCw } from "lucide-react";import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
...
// Add this component after the mock data and before the Schema component (around line 180)
...
const [previewDataState, setPreviewDataState] = useState<Record<string, any[]>>({});
const [relationshipDataState, setRelationshipDataState] = useState<Record<string, { tables: any[], connections: any[] }>>({});
  // Set current selection based on file type
//   const currentSelection = isCsv || isParquet ? selectedFile : isDatabase ? selectedTable : selectedSheet;
// const availableSelections = isCsv || isParquet ? [selectedFile] : isDatabase ? mockTables : mockSheets;
...
useEffect(() => {
  ...
  if (!inputType) {
    console.error("❌ Missing input_type in localStorage");
    toast({
      variant: "destructive",
      title: "Configuration Error",
  ...
  // Remove sheet name from fileKey if embedded (format: path#sheetname)
  ...
  console.log("   cleanFileKey:", cleanFileKey);
  console.log("   finalSheetName:", finalSheetName);

  // Build payload
  const payload: SchemaAnalysisRequest = {
  ...
  console.log("📤 Sending schema analysis request:", payload);

  runSchemaAnalysis(payload)
    .then((res: SchemaAnalysisResponse) => {
      console.log("✅ Schema analysis response:", res);

      if (!res.success || !res.lambda_response?.results) {
        console.error("❌ Schema analysis failed or no results");
        toast({
          variant: "destructive",
          title: "Error",
      ...
      const sheet = Object.keys(results)[0]; // Get first sheet/table name

      console.log("📊 Processing results for sheet:", sheet);
      ...
      console.log("   Current columns:", currentSchemaArr.length);
      console.log("   Previous columns:", previousSchemaArr.length);
    ...
    .catch(err => {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: err instanceof Error ? err.message : "An error occurred while fetching schema data.",
      });
    });
}, [currentSelection, navigate]); // Removed isCsv, isParquet from dependencies
  return (
    <div className="max-w-7xl mt-14 mx-auto p-6 space-y-6">
       {isLoading && (
      <LoadingOverlay message="Loading Schema Analysis..." />
    )}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-medium">{localStorage.getItem("org_data_source")}</span>
          </div>
          {/* {!isCsv && !isParquet && (
            <div className="w-48">
              <Select 
      ...
      <Tabs defaultValue="current" className="w-full">
        ...
        <TabsContent value="current" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Current Schema Structure ({localStorage.getItem("org_data_source")})</CardTitle>
              <CardDescription>
Schema detected from the selected {isDatabase ? 'table' : (isCsv || isParquet) ? (isCsv ? 'CSV file' : 'Parquet file') : 'sheet'}              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
      ...
      <Dialog open={showPreview} onOpenChange={setShowPreview}>
        <DialogContent className="max-w-6xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
Sample data from the selected {isDatabase ? 'table' : (isCsv || isParquet) ? (isCsv ? 'CSV file' : 'Parquet file') : 'sheet'} (showing first 5 rows)            </DialogDescription>
          </DialogHeader>
